...
# Function to export all data to one CSV file
def export_all_to_csv(data: dict) -> None:
    # Define the filename for the large CSV
    filename = "all_zone_reports.csv"
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        # Write the header
        writer.writerow(['Zone', 'Area', 'Average Contact Time (minutes)'])
        
        # Write the data rows for each area in each zone
        for zone, areas in data.items():
            for area, avg_time in areas.items():
                writer.writerow([zone, area, f"{avg_time:.2f}"])
    
    print(f"All results exported to {filename}")

# Main execution: Calculate average contact times and export to one CSV
average_contact_times = calculate_average_contact_time_by_area_and_zone()

# Export the results to one CSV
export_all_to_csv(average_contact_times)
